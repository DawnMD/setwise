...
const mirror = (points: Point[]): Point[] => points.map(([x, y]) => [2 * CENTER - x, y] as Point);
...
function renderSvg(view: "front" | "back", regions: Region[]): string {
  ...
  <!--
    ...
    Every group id below comes from lib/muscles.ts. To swap in a different
...
async function main() {
  ...
  await writeFile(path.join(process.cwd(), "lib", "body-svg.generated.ts"), moduleSource, "utf8");
