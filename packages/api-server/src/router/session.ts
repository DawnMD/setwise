...
export function createSessionRouter(procedure: ProcedureImplementers["protectedApi"]["session"]) {
  return {
    /**
     ...
     */
    active: procedure.active.handler(
     ...
     */
    start: procedure.start.handler(async ({ input, context, errors }) => {
    ...
    logRestDay: procedure.logRestDay.handler(async ({ input, context, errors }) => {
      try {
    ...
    restToday: procedure.restToday.handler(async ({ input, context }) => {
    ...
    recent: procedure.recent.handler(async ({ input, context }) => {
    ...
    createSet: procedure.createSet.handler(async ({ input, context, errors }) => {
    ...
    updateSet: procedure.updateSet.handler(async ({ input, context, errors }) => {
      return context.db.transaction(async (tx) => {
    ...
    deleteSet: procedure.deleteSet.handler(async ({ input, context, errors }) => {
     ...
     */
    finish: procedure.finish.handler(async ({ input, context, errors }) => {
     ...
     */
    discard: procedure.discard.handler(async ({ input, context, errors }) => {
      const session = await findSession(context.db, context.userId, input.id);
      if (!session) throw errors.SESSION_NOT_FOUND();
      if (session.endedAt) throw errors.SESSION_FINISHED();
