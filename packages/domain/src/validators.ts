 ...
 *
 * The bounds below are `@setwise/db`'s column definitions restated in Zod, so a
 * change to `numeric(6, 2)` has one obvious place to land. They sit in the
 * domain rather than beside the schema because both clients check against them
 * before anything reaches the network, and neither may import a database driver
 * to do it. `@setwise/db` imports this file; nothing here imports it back.
 */

import { z } from "zod";

import { ACTIVITY_KINDS } from "./activity";
import { MUSCLE_SLUGS } from "./muscles";
import { ACTIVITY_LEVELS, ageInYears, GOALS, SEXES } from "./nutrition";
