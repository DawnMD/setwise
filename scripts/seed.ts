...
async function main() {
  ...
  try {
    ...
    console.log(`exercise_muscles: ${links.length} rows`);

    if (skipped.length > 0) {
      console.log(`skipped duplicates: ${skipped.slice(0, 10).join(", ")}`);
    }
    ...
    console.log(`global exercises with no muscle mapping: ${orphaned[0]?.count ?? 0}`);
  } finally {
    await client.end();
  }
}
